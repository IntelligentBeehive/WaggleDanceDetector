...
namespace wdd
{
	...
	*/
	...
	void WaggleDanceOrientator::extractUnityOrientationsFromBinaryImage(cv::Mat * td_bw_mat_ptr, std::vector<cv::Vec2d> * unityOrientations_ptr)
	{
		...
		IplImage* td_bw_ipl_ptr = &IplImage(*td_bw_mat_ptr);
	...
	{	
		...
		double angle = cvb::cvAngle(blob_ptr);
	...
	*/
	...
	void WaggleDanceOrientator::showImagesFromFolder(const std::string dirInNameFormat)
	{
		...
		cv::Mat image;
		cv::namedWindow("Image | q or esc to quit", cv::WINDOW_AUTOSIZE | cv::WINDOW_KEEPRATIO | cv::WINDOW_NORMAL);
	...
	void WaggleDanceOrientator::showImage(const cv::Mat *img_ptr)
	{
		...
		cv::namedWindow(winName, cv::WindowFlags::WINDOW_AUTOSIZE | cv::WindowFlags::WINDOW_KEEPRATIO | cv::WindowFlags::WINDOW_NORMAL);
