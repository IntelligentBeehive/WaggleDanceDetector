...
namespace wdd
{
	...
	*/
	...
	{
		return (int) vc->get(cv::CAP_PROP_FRAME_WIDTH);
	}
	int InputVideoParameters::getFrameHeightOf(cv::VideoCapture * vc)
	{
		return (int) vc->get(cv::CAP_PROP_FRAME_HEIGHT);
	}
	...
	{
		return TypeToString::typeToString(static_cast<int>(vc->get(cv::CAP_PROP_FORMAT)));
	}
	// print function(s)
